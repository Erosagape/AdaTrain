...
Const tW_TblName As String = "TTRMUnit"

Private Function SP_SQLtForShow() As String

    Dim tSql As String
    tSql = "SELECT FTUntCode,FTUntName,FTRemark,"
    ...
    If otbCliteria.Text <> "" Then
        ...
        Else
            tSql = tSql & " WHERE ("
            tSql = tSql & "FTUntCode LIKE '%" & otbCliteria.Text & "%' OR "
            tSql = tSql & "FTUntName LIKE '%" & otbCliteria.Text & "%' OR "
    ...
    SP_SQLtForShow = tSql
End Function
Private Function SP_TBLoQueryData(Optional ptFieldKey As String = "FTUntCode") As ADODB.Recordset
    '//Query database for current key input
    Dim oTbl As New ADODB.Recordset
    
    'Set oTbl = SP_TBLoGetUnit(oW_DbConn)
    oTbl.Filter = "[" & ptFieldKey & "]='" & otbFTUntCode.Text & "'"
...
Private Sub SP_DATxSetProperty()
    ...
    Me.otbFTUntCode.MaxLength = 10
    Me.otbFTUntName.MaxLength = 100
...
Private Sub SP_DATxClearForm(Optional ptCode As String = "")
    '//clear form
    Me.otbFTUntCode.Text = ptCode
    Me.otbFTUntName.Text = ""
    Me.otbFTRemark.Text = ""
    Me.otbLastUpdate.Text = ""
    
    Me.otbFTUntCode.Locked = False
    Me.otbFTUntCode.BackColor = vbWhite
...
Private Function SP_TBLbDeleteData() As Boolean
    ...
    tSQLDelete = "DELETE FROM " & tW_TblName & " WHERE FTUntCode=?"
    ...
    oCmd.Parameters.Append oCmd.CreateParameter("p1", adVarChar, adParamInput, 15, Me.otbFTUntCode.Text)
    oCmd.Execute
...
Private Sub ogdMain_Click()
        ...
        otbFTUntName.SetFocus
        
End Sub
Private Sub ogdMain_RowColChange()
    
    If ogdMain.Row > 0 Then
        Me.otbFTUntCode.Text = ogdMain.TextMatrix(ogdMain.Row, 1)
        Call SP_DATxLoadFROMDB
    End If
...
Private Sub SP_DATxLoadFROMDB()
    ...
    If oTbl.EOF = True Then
        ...
        Call SP_DATxClearForm(Me.otbFTUntCode.Text)
    ...
    With oTbl
        Me.otbFTUntCode.Text = "" & .Fields("FTUntCode").Value
        Me.otbFTUntName.Text = "" & .Fields("FTUntName").Value
    ...
    Me.otbFTUntCode.Locked = True
    Me.otbFTUntCode.BackColor = &H8000000F
...
Private Function SP_DATbCheckValidate() As Boolean
    ...
    Dim tMsg As String
    tMsg = ""
    If Trim(otbFTUntCode.Text) = "" Then
        tMsg = tMsg & Split("Unit Code must be input;คุณยังไม่ได้ระบุรหัสหน่วยนับ", ";")(mTRVB.eVB_TRLang) & vbCrLf
    End If
    If Trim(otbFTUntName.Text) = "" Then
        tMsg = tMsg & Split("Unit Name must be input;คุณยังไม่ได้ระบุชื่อหน่วยนับ", ";")(mTRVB.eVB_TRLang) & vbCrLf
    End If
...
Private Sub otbFTUntCode_GotFocus()
    Call SP_CTLxSetFocus(Me.ActiveControl)
End Sub

Private Sub otbFTUntCode_LostFocus()
    If Trim(otbFTUntCode.Text) = "" Then Exit Sub
    Call SP_DATxLoadFROMDB
End Sub

Private Sub otbFTUntName_GotFocus()
    Call SP_CTLxSetFocus(Me.ActiveControl)
End Sub
